...
// 기존 DB 전체 제품명 브랜드 중복 일괄 정리 mutation
export const cleanupDatabaseProductNames = mutation({
    args: {},
    handler: async (ctx) => {
        const products = await ctx.db.query("products").collect();
        let updatedCount = 0;
        for (const p of products) {
            const cleaned = cleanBrandDeduplication(p.name, p.brand);
            if (cleaned !== p.name) {
                await ctx.db.patch(p._id, { name: cleaned, updatedAt: new Date().toISOString() });
                updatedCount++;
            }
        }
